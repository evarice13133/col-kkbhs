...
$hasActiveFilters = (!empty($filters['q'])) 
    ...
    || (!empty($filters['status']) && $filters['status'] !== 'Non inscrit');
?>
<?php if ($hasActiveFilters): ?>
    <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
        ...
        <?php if (!empty($filters['status']) && $filters['status'] !== 'Non inscrit'): ?>
            <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-20 rounded-pill px-3 py-1.5 d-flex align-items-center gap-2 small">
                <span><strong><?= htmlspecialchars($filters['status']) ?></strong></span>
                <a href="javascript:void(0)" onclick="resetFilter('status')" class="text-danger hover-danger text-decoration-none fw-bold" style="font-size: 1.1em; line-height: 1;">&times;</a>
            </span>
        <?php endif; ?>

        <a href="/students/non-inscrits" class="btn btn-link btn-sm text-decoration-none text-muted-theme hover-primary fw-bold p-0 ms-2 small"><?= __('clear_filters') ?? 'Tout effacer' ?></a>
    </div>
<?php endif; ?>
