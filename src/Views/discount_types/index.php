...
<div class="animate-fade-in container-fluid py-3 px-md-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h2 class="fw-black text-main-theme mb-0 fs-4"><?= __('discount_types_title') ?></h2>
            <p class="text-muted-theme small mb-0"><?= __('discount_types_subtitle') ?></p>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4 animate-fade-in">
        <div class="col-6 col-md-4">
            <div class="modern-card kpi-card border-0 shadow-sm">
                ...
                <div class="kpi-label"><?= __('total_categories') ?></div>
            </div>
        </div>
        <div class="col-6 col-md-4">
            <div class="modern-card kpi-card border-0 shadow-sm">
                ...
                <div class="kpi-label"><?= __('active_categories') ?></div>
            </div>
        </div>
        <div class="col-12 col-md-4">
            <div class="modern-card kpi-card border-0 shadow-sm">
                ...
                <div class="kpi-label"><?= __('inactive_categories') ?></div>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div class="d-flex align-items-center flex-wrap gap-3">
            <div class="input-group search-pill bg-white bg-opacity-10 rounded-pill px-2 border-theme-light" style="max-width: 280px; border: 1px solid var(--border-color);">
                ...
                <input type="text" id="type-search-input" class="form-control border-0 bg-transparent shadow-none py-1.5 small text-main" placeholder="<?= __('quick_search_placeholder') ?>" style="font-size: 0.85rem;">
            </div>
        </div>

        <div>
            <button type="button" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm" id="btn-add-type">
                <i class="bi bi-plus-circle-fill me-2"></i> <?= __('new_type') ?>
            </button>
        </div>
    </div>
    ...
    <div class="modern-card border-0 shadow-sm overflow-hidden animate-fade-in">
        <div class="table-responsive">
            <table class="table-modern" id="types-table">
                <thead>
                    <tr>
                        <th class="ps-4"><?= __('type_name') ?></th>
                        <th><?= __('description') ?></th>
                        <th><?= __('admin_comment') ?></th>
                        <th><?= __('date_creation') ?></th>
                        <th><?= __('creator') ?></th>
                        ...
                        <?php foreach ($discountTypes as $type): ?>
                            <tr class="type-row" data-id="<?= $type['id'] ?>">
                                ...
                                <td class="text-center">
                                    <span style="cursor: pointer;" class="badge-premium <?= $type['status'] === 'active' ? 'badge-premium-success' : 'badge-premium-secondary' ?> toggle-status-btn" data-id="<?= $type['id'] ?>" title="<?= __('toggle_status') ?>">
                                        <?php if ($type['status'] === 'active'): ?>
                                            <i class="bi bi-check-circle-fill"></i> <?= __('active') ?>
                                        <?php else: ?>
                                            <i class="bi bi-x-circle-fill"></i> <?= __('inactive') ?>
                                        <?php endif; ?>
                                    </span>
                                </td>
                                <td class="pe-4 text-center">
                                    <div class="d-flex gap-1 justify-content-center">
                                        ...
                                        <button type="button" class="btn btn-sm btn-action-modern text-danger delete-type-btn" data-id="<?= $type['id'] ?>" title="<?= __('delete') ?>">
        ...
        <div class="modal-content border-0 rounded-4 shadow">
            <form id="discountTypeForm">
                ...
                <div class="modal-header border-bottom-0 pb-0 px-4 pt-4">
                    <h5 class="modal-title fw-black text-main-theme" id="discountTypeModalLabel"><?= __('new_type_title') ?></h5>
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        <div class="col-12">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('type_name') ?> *</label>
                            <input type="text" name="name" id="type_name" class="form-control premium-input fw-bold" placeholder="<?= __('type_name_placeholder') ?>" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('description') ?></label>
                            <textarea name="description" id="type_description" class="form-control premium-input" rows="3" placeholder="<?= __('description_placeholder') ?>"></textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('internal_comment') ?></label>
                            <textarea name="comment" id="type_comment" class="form-control premium-input" rows="2" placeholder="<?= __('internal_comment_placeholder') ?>"></textarea>
                        </div>
                        <div class="col-12">
                            ...
                            <select name="status" id="type_status" class="form-select premium-input" required>
                                <option value="active" selected><?= __('active_available') ?></option>
                                <option value="inactive"><?= __('inactive_unavailable') ?></option>
                            </select>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btn-add-type').addEventListener('click', function() {
        labelTitle.textContent = "<?= __('new_type_title') ?>";
        fieldId.value = "0";
        form.reset();
        modal.show();
    });
    ...
    document.addEventListener('click', function(e) {
        ...
        if (btnEdit) {
            e.preventDefault();
            labelTitle.textContent = "<?= __('edit_type') ?>";
    ...
    form.addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.success) {
                ...
                AlertService.error('<?= __('error_title') ?>', data.message);
            }
        })
        .catch(error => {
            console.error('Error:', error);
            AlertService.toast('error', '<?= __('communication_error') ?>');
        });
    });
    ...
    document.addEventListener('click', function(e) {
        ...
        if (btnToggle) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.status === 'active') {
                        ...
                        btnToggle.innerHTML = '<i class="bi bi-check-circle-fill"></i> <?= __('active') ?>';
                        ...
                        btnToggle.innerHTML = '<i class="bi bi-x-circle-fill"></i> <?= __('inactive') ?>';
            ...
            .catch(error => {
                console.error('Error:', error);
                AlertService.toast('error', '<?= __('status_error') ?>');
            });
        }
    });
    ...
    document.addEventListener('click', function(e) {
        ...
        if (btnDelete) {
            ...
            AlertService.confirmDelete(
                '<?= __('confirm_delete_type_title') ?>',
                '<?= __('confirm_delete_type_text') ?>'
            ).then((result) => {
                if (result.isConfirmed) {
                    ...
                    .catch(error => {
                        console.error('Error:', error);
                        AlertService.toast('error', '<?= __('delete_error') ?>');
                    });
                }
            });
        }
