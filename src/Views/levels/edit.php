<?php $title = __('edit_level') ?? 'Modifier le Niveau'; ob_start(); ?>

<div class="animate-fade-in container-fluid py-4 max-w-700">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <h2 class="fw-black text-main-theme m-0 fs-4">
            <i class="bi bi-pencil-fill me-2 text-primary"></i><?= __('edit_level') ?? 'Modifier le Niveau' ?>
        </h2>
    ...
    <div class="modern-card border-0 shadow-sm overflow-hidden">
        <div class="card-body p-4">
            <form action="/levels/update?id=<?= $level['id'] ?>" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="teaching_type_id" class="form-select premium-input border-primary border-opacity-25" required>
                        ...
                        <?php foreach ($teachingTypes as $tt): ?>
                            <option value="<?= $tt['id'] ?>" <?= (int)($level['teaching_type_id'] ?? 0) === (int)$tt['id'] ? 'selected' : '' ?>>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="code" class="form-control premium-input text-uppercase" placeholder="Ex: SIL, 6EME, L1" value="<?= htmlspecialchars((string)($level['code'] ?? '')) ?>" required>
                </div>

                <div class="mb-3">
                    <label class="form-label fw-bold extra-small text-uppercase text-muted-theme"><?= __('label_fr') ?? 'Libellé (Français)' ?> *</label>
                    <input type="text" name="libelle_fr" class="form-control premium-input" placeholder="Ex: Section d'Initiation à la Lecture" value="<?= htmlspecialchars((string)($level['libelle_fr'] ?? '')) ?>" required>
                </div>

                <div class="mb-3">
                    <label class="form-label fw-bold extra-small text-uppercase text-muted-theme"><?= __('label_en') ?? 'Libellé (Anglais)' ?> *</label>
                    <input type="text" name="libelle_en" class="form-control premium-input" placeholder="Ex: Class 1 / Level 1" value="<?= htmlspecialchars((string)($level['libelle_en'] ?? '')) ?>" required>
                </div>

                <div class="form-check form-switch mt-3 mb-4">
                    <input class="form-check-input" type="checkbox" name="status" id="edit_status" value="1" <?= (int)($level['status'] ?? 1) === 1 ? 'checked' : '' ?>>
                    <label class="form-check-label fw-bold text-main-theme" for="edit_status"><?= __('active') ?? 'Actif' ?></label>
                </div>
