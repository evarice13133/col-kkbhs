<?php $title = __('assignments') . ": " . htmlspecialchars($teacher['nom']);
ob_start(); ?>
...
<div class="animate-fade-in container-fluid py-4">
        ...
        <div>
            ...
            <div class="d-flex align-items-center gap-2 mt-1">
                ...
                <span class="text-secondary small fw-bold text-main-theme">
                    ...
                    <span class="text-primary fw-extrabold"><?= count($assignedSubjectsMap) ?></span>
                    <?= __('subjects') ?>
                </span>
            </div>
        </div>
        <div class="d-flex gap-2">
            <button type="submit" form="mainAssignmentForm"
                class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm transition-base scale-on-hover"
                <?= $isHistoricalView ? 'disabled' : '' ?>>
                <i class="bi bi-check2-circle me-1 fs-6"></i> <?= __('save') ?>
            </button>
            <a href="/teachers"
                class="btn btn-light-theme rounded-pill px-4 border border-theme-light shadow-sm transition-base text-main-theme fw-bold">
    ...
    <div class="d-flex justify-content-center mb-5 px-2">
        <div class="filter-island px-4 py-3 shadow-lg animate-slide-down w-100" style="max-width: 800px;">
            <div class="d-flex align-items-center gap-3 flex-wrap flex-md-nowrap w-100">
                ...
                <div class="input-group year-pill bg-white bg-opacity-10 rounded-pill px-2 flex-grow-1"
                    style="max-width: 320px;">
                    <span class="input-group-text border-0 bg-transparent text-primary">
                        <i class="bi bi-calendar3"></i>
                    </span>
                    <select id="academic_year_selector"
                        class="form-select border-0 bg-transparent shadow-none py-2 text-main-theme fw-bold"
                ...
                <div class="input-group search-pill bg-white bg-opacity-10 rounded-pill px-2 flex-grow-1">
                    ...
                    <input type="text" id="assignment-search"
                        class="form-control border-0 bg-transparent shadow-none py-2 text-main-theme"
    ...
    <div class="mb-4 px-2">
        <ul class="nav nav-pills custom-pills-modern" id="assignmentTabs" role="tablist">
            <li class="nav-item">
                <button class="nav-link active" id="current-tab" data-bs-toggle="pill" data-bs-target="#tab-current"
            ...
            <?php if (!$isHistoricalView): ?>
                <li class="nav-item">
                    <button class="nav-link" id="catalog-tab" data-bs-toggle="pill" data-bs-target="#tab-catalog"
                        type="button" role="tab">
                ...
                <?php else: ?>
                    <div class="row g-4 px-2">
                        <?php foreach ($assignedSubjectsMap as $sub_id => $data): ?>
                            <div class="col-md-6 col-xl-4 searchable"
                                data-search="<?= strtolower(($data['nom'] ?? '') . ' ' . implode(' ', array_column($data['classes'] ?? [], 'nom'))) ?>">
                                <div class="modern-card border-0 shadow-sm rounded-4 h-100 overflow-hidden">
                                    <div
                                        class="card-header bg-primary bg-opacity-10 border-0 py-3 px-4 d-flex justify-content-between align-items-center">
                                        <h6 class="fw-bold mb-0 text-primary text-uppercase small"
                                            style="letter-spacing: 0.5px;"><?= htmlspecialchars($data['nom']) ?></h6>
                                        <div class="form-check form-switch p-0">
                                            <input class="form-check-input subject-toggle ms-0 cursor-pointer" type="checkbox"
                                                checked data-target=".group-current-<?= $sub_id ?>">
                                        </div>
                                    </div>
                                    <div class="p-4 pt-3">
                                        <div class="d-grid gap-2">
                                            <?php foreach ($data['classes'] as $cls):
                                                ...
                                                <div class="d-flex align-items-center justify-content-between gap-2 p-1">
                                                    <label class="class-item selected flex-grow-1 mb-0"
                                                        for="asg_current_<?= $pair_key ?>">
                                                        <input
                                                            class="form-check-input group-current-<?= $sub_id ?> d-none js-asg-checkbox"
                                                            type="checkbox" name="assignments[]" value="<?= $pair_key ?>"
                                                            id="asg_current_<?= $pair_key ?>" checked>
                                                        <div class="checkbox-custom"></div>
                                                        <span
                                                            class="small fw-bold text-main-theme"><?= htmlspecialchars($cls['nom']) ?></span>
                                                    </label>
                                                    <?php if (!$isHistoricalView): ?>
                                                        <button type="button"
                            ...
                            <div class="col-md-6 col-xl-4 searchable" data-search="<?= htmlspecialchars($searchIndex) ?>">
                                <div class="modern-card border-0 shadow-sm rounded-4 h-100 overflow-hidden">
                                    <div
                                        ...
                                        <h6 class="fw-bold mb-0 text-main-theme text-uppercase small"
                                            style="letter-spacing: 0.5px;"><?= htmlspecialchars($data['nom'] ?? 'Inconnu') ?>
                                        </h6>
                                        <div class="form-check form-switch p-0">
                                            <input class="form-check-input subject-toggle ms-0 cursor-pointer" type="checkbox"
                                                data-target=".group-avail-<?= $sub_id ?>">
                                        </div>
                                    </div>
                                    <div class="p-4 pt-3">
                                        <div class="d-grid gap-2">
                                            <?php foreach ($data['classes'] as $cls):
                                                ...
                                                <label class="class-item <?= $is_taken ? 'is-taken' : '' ?>" <?= !$is_taken ? 'for="asg_' . $pair_key . '"' : '' ?>>
                                                    <?php if (!$is_taken): ?>
                                                        <input
                                                            class="form-check-input group-avail-<?= $sub_id ?> d-none js-asg-checkbox"
                                                            type="checkbox" name="assignments[]" value="<?= $pair_key ?>"
                                                            id="asg_<?= $pair_key ?>">
                                                        <div class="checkbox-custom"></div>
                                                    <?php else: ?>
...
<div class="modal fade" id="transferCourseModal" tabindex="-1" aria-labelledby="transferCourseModalLabel"
    aria-hidden="true" style="z-index: 1085;">
                ...
                <div class="modal-body p-4">
                    <div class="alert alert-warning border-0 rounded-3 p-3 mb-4">
                        <div class="d-flex align-items-start gap-2">
                            ...
                            <div>
                                <h6 class="fw-bold mb-1">Un même cours ne peut pas être affecté à deux enseignants
                                    simultanément.</h6>
                                <p class="small mb-0 opacity-85">
                                    Ce cours est déjà attribué à un autre enseignant. Pour lui attribuer un nouveau
                                    titulaire, vous devez effectuer le transfert des données et de l'affectation.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="card border border-warning border-opacity-30 rounded-3 p-3 mb-4 bg-warning bg-opacity-10">
                        <div class="row g-3 text-main-theme">
                            <div class="col-md-4">
                                <span class="text-muted extra-small d-block text-uppercase fw-bold">Matière
                                    concernée</span>
                                <strong id="tr_subject_name_text" class="text-primary fs-6"></strong>
                            </div>
                            <div class="col-md-4">
                                <span class="text-muted extra-small d-block text-uppercase fw-bold">Classe
                                    concernée</span>
                                <strong id="tr_class_name_text" class="text-main-theme fs-6"></strong>
                            </div>
                            <div class="col-md-4">
                                <span class="text-muted extra-small d-block text-uppercase fw-bold">Enseignant
                                    Actuel</span>
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-bold text-main-theme small">
                            <i class="bi bi-person-check-fill me-1 text-primary"></i>Choisir le nouveau titulaire du
                            cours :
                        </label>
                        <select id="tr_target_teacher_id" name="target_teacher_id" class="form-select rounded-3">
                            ...
                            <?php foreach ($allTeachers as $t_opt): ?>
                                <option value="<?= $t_opt['id'] ?>" <?= $t_opt['id'] == $teacher['id'] ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($t_opt['nom'] . ' ' . $t_opt['prenom']) ?>
                                    (<?= htmlspecialchars($t_opt['username']) ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="p-3 rounded-3 border bg-light-theme mb-3">
                        <div class="form-check form-switch">
                            <input class="form-check-input cursor-pointer" type="checkbox" id="create_new_teacher"
                                name="create_new_teacher" value="1" onchange="toggleNewTeacherFields(this)">
                            <label class="form-check-label fw-bold text-main-theme small cursor-pointer"
                                for="create_new_teacher">
                                <i class="bi bi-person-plus-fill me-1 text-success"></i>Créer directement un nouvel
                                enseignant et lui transférer ce cours
                            </label>
                        </div>
                        <div id="newTeacherInputContainer" class="mt-3 d-none">
                            <label class="form-label fw-bold extra-small text-main-theme mb-1">Nom et Prénom du nouvel
                                enseignant *</label>
                            <input type="text" name="new_teacher_name" id="tr_new_teacher_name"
                                class="form-control rounded-3" placeholder="Ex: Dr. Martin KAMGA">
                        </div>
                    </div>
