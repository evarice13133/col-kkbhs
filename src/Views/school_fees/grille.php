...
<div class="animate-fade-in container-fluid py-3 px-md-4">
    ...
    <div
        ...
        <div
            class="d-flex flex-column flex-md-row gap-2 ms-md-auto mt-3 mt-md-0 align-items-stretch align-items-md-center justify-content-md-end w-100 w-md-auto">
            <button type="button" class="btn btn-outline-primary rounded-pill px-3 fw-bold text-center text-nowrap"
                data-bs-toggle="modal" data-bs-target="#importGrilleModal">
                <i class="bi bi-file-earmark-arrow-up me-1"></i> <?= __('btn_import') ?>
            </button>
            <a href="/school_fees/grille/print<?= !empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : '' ?>"
                target="_blank" class="btn btn-theme-soft rounded-pill px-3 fw-bold text-center text-nowrap">
                <i class="bi bi-printer me-1"></i> <?= __('print_pdf') ?>
            </a>
            <button id="btn-export-excel"
                class="btn btn-outline-success rounded-pill px-3 fw-bold text-center text-nowrap">
    ...
    <div class="modern-card border-0 shadow-sm p-4 mb-4">
        <form method="GET" class="row g-3 align-items-end">
            <div class="col-md-3">
                <label
                    class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('teaching_type') ?></label>
                <select name="teaching_type_id" class="form-select premium-select" onchange="this.form.submit()">
                    ...
                    <?php foreach ($teachingTypes as $tt): ?>
                        <option value="<?= $tt['id'] ?>" <?= $teachingTypeId === (int) $tt['id'] ? 'selected' : '' ?>>
                            <?= h($tt['nom']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="col-md-3">
                <label
                    class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('cycle') ?></label>
                <select name="cycle_id" class="form-select premium-select" onchange="this.form.submit()">
                    <option value=""><?= __('all_m') ?></option>
                    <?php foreach ($cycles as $c): ?>
                        <option value="<?= $c['id'] ?>" <?= $cycleId === (int) $c['id'] ? 'selected' : '' ?>>
                            <?= h($c['nom']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="col-md-3">
                <label
                    class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('class_section') ?></label>
                <select name="section_id" class="form-select premium-select" onchange="this.form.submit()">
                    <option value=""><?= __('all_f') ?></option>
                    <?php foreach ($sections as $s): ?>
                        <option value="<?= $s['id'] ?>" <?= $sectionId === (int) $s['id'] ? 'selected' : '' ?>>
                            <?= h($s['nom']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="col-md-3 d-flex gap-2">
                <div class="w-100">
                    <label
                        class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('class') ?></label>
                        ...
                        <?php foreach ($grilleData as $row): ?>
                            <tr>
                                ...
                                <td class="text-end fw-bold">
                                    <?= number_format($row['frais_inscription_nouveau'], 0, '.', ' ') ?> <span
                                        class="extra-small text-muted">FCFA</span>
                                </td>
                                <td class="text-end fw-bold">
                                    <?= number_format($row['frais_inscription_ancien'], 0, '.', ' ') ?> <span
                                        class="extra-small text-muted">FCFA</span>
                                </td>
                                <td class="text-end fw-black text-primary">
                                    <?= number_format($row['frais_scolarite_brut'], 0, '.', ' ') ?> <span
                                        class="extra-small text-muted">FCFA</span>
                                </td>
                                <td class="text-center">
                                    <span
                                        class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-10 px-2 py-1 rounded-pill fw-bold">
                                        <?= $row['nbr_tranches'] ?>
                                    </span>
                                </td>
                                <td>
                                            ...
                                            <?php foreach ($row['tranches'] as $tr): ?>
                                                <div class="badge-premium badge-premium-info"
                                                    ...
                                                    <span class="ms-1 border-start border-secondary border-opacity-25 ps-1 text-muted"
                                                        ...
                                                        <i
                                                            class="bi bi-calendar-event me-1"></i><?= date('d/m/Y', strtotime($tr['deadline_date'])) ?>
                                                    </span>
                                                </div>
                                            <?php endforeach; ?>
                                        </div>
...
<!-- Modal d'Importation -->
<div class="modal fade" id="importGrilleModal" tabindex="-1" aria-labelledby="importGrilleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <form
                action="/school_fees/grille/import<?= !empty($_SERVER['QUERY_STRING']) ? '?' . htmlspecialchars($_SERVER['QUERY_STRING'], ENT_QUOTES, 'UTF-8') : '' ?>"
                method="POST" enctype="multipart/form-data" class="no-loader" id="importGrilleForm">
                <div class="modal-body p-4">
                    ...
                    <div class="mb-4 text-center">
                        ...
                        <a href="/school_fees/grille/template<?= !empty($_SERVER['QUERY_STRING']) ? '?' . htmlspecialchars($_SERVER['QUERY_STRING'], ENT_QUOTES, 'UTF-8') : '' ?>"
                    ...
                    <div class="mb-3 text-start">
                        <label
                            class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-2"><?= __('select_excel_csv_file') ?></label>
                        <!-- Stylized Drag & Drop Area -->
                        <div class="drag-drop-zone border-2 border-dashed border-primary border-opacity-25 rounded-4 p-4 text-center cursor-pointer position-relative bg-light bg-opacity-50 hover-bg-opacity-100"
                            id="dropzone" style="border-style: dashed !important; border-width: 2px !important;">
                            <input type="file" name="import_file" id="import_file_input"
                                class="position-absolute top-0 start-0 w-100 h-100 opacity-0 cursor-pointer"
                                accept=".xlsx, .xls, .csv" required style="z-index: 10;">
                            <div class="py-2" style="pointer-events: none;">
                                <i class="bi bi-cloud-arrow-up text-primary fs-2 mb-2 d-block"></i>
                                <span class="fw-bold d-block text-main-theme small"
                                    id="filename-preview"><?= __('drag_drop_preview') ?></span>
                                <span
                                    class="text-muted extra-small d-block mt-1"><?= __('accepted_files_hint') ?></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer border-0 bg-light py-3 d-flex justify-content-between">
                    <button type="button" class="btn btn-light rounded-pill px-3"
                        data-bs-dismiss="modal"><?= __('btn_cancel') ?></button>
