<?php $title = __('subject_groups') ?? 'Groupes de Modules'; ob_start(); ?>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-column flex-md-row gap-3">
        <form method="GET" action="/subject-groups" class="d-flex gap-2 align-items-center w-100 w-md-auto flex-wrap">
            <div class="input-group search-pill bg-white bg-opacity-10 rounded-pill px-2" style="max-width: 250px;">
                ...
                <input type="text" name="q" id="instantSearchInput" class="form-control border-0 bg-transparent shadow-none py-2 text-main"
                    value="<?= htmlspecialchars((string) $filters['q']) ?>"
                    placeholder="<?= __('search') ?>...">
            </div>

            <select name="teaching_type_id" class="form-select rounded-pill px-3 py-2 border-theme-light" style="max-width: 220px;" onchange="this.form.submit()">
        ...
        <button type="button" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm scale-on-hover text-nowrap" onclick="openCreateGroupModal()">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($groups as $g): ?>
                            <tr class="group-row">
                                <td class="ps-4">
                                    <span class="fw-bold text-main-theme group-libelle"><?= htmlspecialchars((string) $g['libelle']) ?></span>
                                </td>
                                <td>
                                    <span class="badge bg-secondary bg-opacity-10 text-secondary fw-bold px-3 py-1 rounded-pill small">
                                        <i class="bi bi-diagram-3 me-1"></i><?= htmlspecialchars((string) ($g['teaching_type_nom'] ?? 'N/A')) ?>
                                    </span>
                                </td>
                                <td>
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-1">
                                        <a href="/subject-groups/toggle?id=<?= $g['id'] ?>"
                                            ...
                                            <i class="bi <?= (int) $g['status'] === 1 ? 'bi-pause-circle' : 'bi-play-circle' ?> fs-5"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-action-modern text-primary border-0 bg-transparent"
                                            onclick="openEditGroupModal(<?= htmlspecialchars(json_encode([
                                                'id' => (int)$g['id'],
                                                'libelle' => $g['libelle'],
        ...
        <div class="modal-content rounded-4 border-0 shadow-lg overflow-hidden" style="background: var(--bg-card);">
            <div class="modal-header border-bottom-0 pb-0 px-4 pt-4">
                <div class="d-flex align-items-center gap-3">
                    <div class="avatar-init bg-primary text-white rounded-4 d-flex align-items-center justify-content-center shadow-sm" style="width: 44px; height: 44px;">
                        <i class="bi bi-collection fs-4"></i>
                    </div>
                    <div>
                        <h5 class="modal-title fw-black text-main-theme" id="subjectGroupModalLabel">Nouveau Groupe de Modules</h5>
            ...
            <form id="subjectGroupForm" action="/subject-groups/store" method="POST">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group-modern">
                            ...
                            <input type="text" name="libelle" id="group_libelle" class="form-control premium-input" placeholder="Ex: Groupe 1 - Matières Littéraires" required>
                        </div>
                    </div>
                    ...
                    <div class="form-check form-switch mt-3" id="group_status_wrapper">
                        <input class="form-check-input" type="checkbox" name="status" id="group_status" value="1" checked>
                ...
                <div class="modal-footer border-top-0 pt-0 px-4 pb-4 gap-2">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm scale-on-hover">
                        <i class="bi bi-check-circle-fill me-2"></i> <span id="groupSubmitBtnText"><?= __('save') ?></span>
                    </button>
                </div>
            </form>
        </div>
