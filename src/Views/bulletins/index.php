...
<div class="animate-fade-in admin-analytics module-bureau-flow">
    ...
    <div class="d-flex justify-content-center mb-5">
        <div class="filter-island px-3 py-2 shadow-lg animate-slide-down" style="min-width: 70%;">
            <form method="GET" action="/bulletins" class="d-flex align-items-center gap-3 flex-wrap flex-md-nowrap filter-form w-100">
                ...
                <div class="d-flex gap-2 align-items-center border-start border-opacity-10 border-secondary ps-3">
                     <a href="/bulletins" class="btn btn-light rounded-circle p-2 d-flex align-items-center justify-content-center reset-btn" style="width: 40px; height: 40px;" title="<?= __('reset') ?>">
            ...
            <div class="modern-card-body p-4 p-lg-5">
                <form id="unifiedForm" target="_blank" action="#" method="GET">
                    ...
                    <div class="flow-step mb-5">
                        ...
                        <h5 class="fw-bold mb-3"><?= __('select_bulletin_type') ?></h5>
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="w-100 bulletin-type-card m-0" data-type="sequence">
                                    ...
                                    <div
                                        class="card card-body text-center shadow-sm hover-elevate border-light transition-all rounded-4">
                                        <i class="bi bi-layout-text-sidebar-reverse fs-1 text-info mb-2"></i>
                                        <h6 class="fw-bold m-0"><?= __('sequence_bulletin') ?></h6>
                                    </div>
                                </label>
                            </div>
                            <div class="col-md-4">
                                <label class="w-100 bulletin-type-card m-0" data-type="trimestre">
                                    ...
                                    <div
                                        ...
                                        <h6 class="fw-bold m-0"><?= __('term_bulletin') ?></h6>
                                    </div>
                                </label>
                            </div>
                            <div class="col-md-4">
                                <label class="w-100 bulletin-type-card m-0" data-type="annuel">
                                    ...
                                    <div
                                        ...
                                        <h6 class="fw-bold m-0"><?= __('annual_bulletin') ?></h6>
                                    </div>
                                </label>
                            </div>
                        </div>
                    ...
                    <div id="dynamic-detail-section" class="flow-step mb-5 d-none">
                        ...
                        <div id="sequence-select" class="d-none animate-fade-in">
                            ...
                            <select name="sequence_id" id="sequence_id_input"
                                class="form-select form-select-lg rounded-4 shadow-sm">
                                <option value=""><?= __('choose_sequence') ?></option>
                                <?php foreach ($sequences as $sequence): ?>
                                    <option value="<?= $sequence['id'] ?>"><?= htmlspecialchars((string) $sequence['label']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div id="target-section" class="flow-step mb-5 d-none">
                        ...
                        <div class="d-flex gap-2 flex-wrap mb-4">
                            <div class="form-check p-0 m-0 flex-grow-1" style="min-width: 200px;">
                                <input type="radio" class="btn-check" name="target_type" id="target_student" value="student"
                                    required>
                                <label
                                    class="btn btn-outline-primary w-100 p-3 rounded-4 fw-bold shadow-sm text-center transition-all"
                                    for="target_student">
                        ...
                        <div id="student-select" class="d-none animate-fade-in shadow-sm p-3 border rounded-4 bg-light">
                            <select name="student_id" id="student_id_input"
                                class="form-select form-select-lg rounded-4 shadow-sm">
                                <option value=""><?= __('choose_student') ?></option>
                                <?php foreach ($students as $student): ?>
                                    <option value="<?= $student['id'] ?>"><?= htmlspecialchars((string) $student['nom']) ?>
                                        <?= htmlspecialchars((string) $student['prenom']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
        ...
        function updateState() {
            ...
            if (selectedType) {
                ...
                if (selectedType === 'annuel') {
                    ...
                    if (selectedType === 'sequence') sequenceSelectDiv.classList.remove('d-none');
                    else if (selectedType === 'trimestre') termSelectDiv.classList.remove('d-none');
                }
            }

            if (selectedTarget === 'student') studentSelectDiv.classList.remove('d-none');
            else studentSelectDiv.classList.add('d-none');
            ...
            if (isValid) {
                ...
                generateText.textContent = "<?= __('generate') ?>"; // Simplified or dynamic
            } else {
                generateText.textContent = "<?= __('complete_selections') ?>";
            }

            if (isValid) {
                let baseUrl = '/bulletins/';
