                    ...
                    <?php else: ?>
                        <?php foreach ($sequences as $s): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-secondary bg-opacity-10 text-secondary fw-bold px-3 py-1 rounded-pill small">
                                        <i class="bi bi-diagram-3 me-1"></i><?= htmlspecialchars((string) ($s['teaching_type_nom'] ?? 'N/A')) ?>
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <?php elseif ($isSecondary && !empty($s['trimestre'])): ?>
                                        <div class="p-1 px-2 rounded-2 bg-info bg-opacity-10 text-info small fw-bold d-inline-block">
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-1">
                                        <a href="/sequences/toggle?id=<?= $s['id'] ?>"
                                            ...
                                            <i class="bi <?= (int) $s['is_active'] === 1 ? 'bi-pause-circle' : 'bi-play-circle' ?> fs-5"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-action-modern text-primary border-0 bg-transparent"
                                            onclick="openEditEvaluationModal(<?= htmlspecialchars(json_encode([
                                                'id' => (int)$s['id'],
        ...
        <div class="modal-content rounded-4 border-0 shadow-lg overflow-hidden" style="background: var(--bg-card);">
            <div class="modal-header border-bottom-0 pb-0 px-4 pt-4">
                <div class="d-flex align-items-center gap-3">
                    <div class="avatar-init bg-primary text-white rounded-4 d-flex align-items-center justify-content-center shadow-sm" style="width: 44px; height: 44px;" id="evalModalIcon">
                        <i class="bi bi-journal-plus fs-4"></i>
                    </div>
                    <div>
                        <h5 class="modal-title fw-black text-main-theme" id="evaluationModalLabel"><?= __('add_new_evaluation') ?></h5>
                        <p class="text-muted-theme small mb-0 opacity-75" id="evalModalSubtext">Définissez les paramètres de l'évaluation</p>
                    </div>
                </div>
            ...
            <form id="evaluationForm" action="/sequences/store" method="POST">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group-modern">
                            ...
                            <select name="teaching_type_id" id="eval_teaching_type_id" class="form-select premium-input" required onchange="handleTeachingTypeChange()">
                                <option value="" disabled selected>Sélectionner un type...</option>
                                <?php foreach ($teachingTypes as $tt): ?>
                                    <option value="<?= $tt['id'] ?>" data-code="<?= htmlspecialchars($tt['code']) ?>" data-nom="<?= htmlspecialchars($tt['nom']) ?>">
                    ...
                    <div id="bloc_secondaire" class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('sequence_code') ?> *</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-hash"></i></span>
                                <input type="text" name="code" id="eval_code_sec" class="form-control premium-input" placeholder="Ex: SEQ1">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('position_order') ?> *</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-sort-numeric-down"></i></span>
                                <input type="number" name="position" id="eval_position_sec" class="form-control premium-input" placeholder="Ex: 1">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('sequence_label') ?> *</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-fonts"></i></span>
                                <input type="text" name="label" id="eval_label_sec" class="form-control premium-input" placeholder="Ex: Trimestre 1 - Séquence 1">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('Short_Label') ?> *</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-tag"></i></span>
                                <input type="text" name="short_label" id="eval_short_label_sec" class="form-control premium-input" placeholder="Ex: SEQ 1" maxlength="20">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1"><?= __('trimester') ?> *</label>
                            <div class="input-group-modern">
                    ...
                    <div id="bloc_superieur" class="row g-3 d-none">
                        <div class="col-md-4">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1">Code de l'évaluation *</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-hash"></i></span>
                                <input type="text" name="code" id="eval_code_sup" class="form-control premium-input" placeholder="Ex: CC1, EXAMEN_S1">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1">Libellé de l'évaluation *</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-fonts"></i></span>
                                <input type="text" name="label" id="eval_label_sup" class="form-control premium-input" placeholder="Ex: Contrôle Continu Semestre 1">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1">Libellé court *</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-tag"></i></span>
                                <input type="text" name="short_label" id="eval_short_label_sup" class="form-control premium-input" placeholder="Ex: CC S1" maxlength="20">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1">Ordre de position *</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-sort-numeric-down"></i></span>
                                <input type="number" name="position" id="eval_position_sup" class="form-control premium-input" placeholder="Ex: 1">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1">Période : Date de début</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-calendar-event"></i></span>
                                <input type="date" name="start_date" id="eval_start_date_sup" class="form-control premium-input">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label text-muted-theme fw-bold extra-small text-uppercase mb-1">Période : Date de fin</label>
                            <div class="input-group-modern">
                                <span class="input-group-text-modern"><i class="bi bi-calendar-check"></i></span>
                                <input type="date" name="end_date" id="eval_end_date_sup" class="form-control premium-input">
                            </div>
                        </div>
                    </div>

                    <div class="form-check form-switch mt-4" id="eval_active_wrapper">
                        <input class="form-check-input" type="checkbox" name="is_active" id="eval_is_active" value="1" checked>
                ...
                <div class="modal-footer border-top-0 pt-0 px-4 pb-4 gap-2">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm scale-on-hover">
                        <i class="bi bi-check-circle-fill me-2"></i> <span id="evalSubmitBtnText"><?= __('save') ?></span>
                    </button>
                </div>
            </form>
        </div>
